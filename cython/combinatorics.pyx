...
def Fibonacci(int n): #Считает n-ое число Фибоначчи
    сdef int a, b, c, i #Объявление переменных
    if n < 1: #Возвращает -1, если аргумент неправильный
        ...
        for i in range(2, n): #Основной цикл
...
def PrimeQ(int n): #Проверяет целое число на простоту
...
def Prime(int n): #Возвращет n-е простое число
    if n < 1: #Возвращает -1, если аргумент неправильный
        return -1
...
def PowerMod(int a, int n, int m): #Возводит в степень по модулю m
    if (n == 1)
        return a % m
    int b
    if (n % 2 == 0)
...
def DiscreteLog(int a, int b, int m): #Логарифмирование в кольце вычетов
    cdef int c, n
    c = a
    n = 1
    while c != b:
